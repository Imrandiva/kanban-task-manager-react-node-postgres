...
export default class KanbanBoard extends Component {
  ...
  async createMerchant(name, course, due_date, stage) {
...
// }
  ...
  update = (name) => {
    ...
    for (let i =0; i < Object.keys(_tasks).length; i++) {
     if (_tasks[i].name == name) {
      if (column == "name") {
        this.createMerchant(change, _tasks[i].course, _tasks[i].chosenDate, _tasks[i].stage)
        this.remove(name)

      }
      else if (column == "course") {
        this.createMerchant(_tasks[i].name ,change, _tasks[i].chosenDate,  _tasks[i].stage)
        this.remove(name)

      }
      else if (column == "date") {
        this.createMerchant(_tasks[i].name , _tasks[i].course, change,  _tasks[i].stage)
        this.remove(name)
